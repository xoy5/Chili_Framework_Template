...
public:
	...
	void Draw(Graphics& gfx) const
	{
		...
		// padding
		Vei2 textPos = pos + Vei2{sizePaddingX, sizePaddingY};
		// positionCenter
		if (isSetPositionCenter)
		{
			textPos -= Vei2{sizeWidth / 2 + sizePaddingX, sizeHeight / 2 + sizePaddingY};
		}
		// textAlignCenter
	...
	bool IsClicked(const Mouse::Event& m) const
	{
		return hovered && m.LeftIsPressed();
	}
...
private:
	RectI GetRect() const
	{
		if (isSetPositionCenter == false)
			return RectI(pos, sizePaddingX + sizeWidth + sizePaddingX, sizePaddingY + sizeHeight + sizePaddingY);
